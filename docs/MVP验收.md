...
该 MVP 面向 AI 短剧、漫剧和虚拟角色生产，在角色图进入分镜、视频生成和宣发前，检索其与公众人物参考库的脸部特征近似，向人工审核人员提供 Top-K 候选、相似度和图片来源。

验收目标是证明“AI 角色图上传 → 人脸检测 → 公众人物候选检索 → 来源回溯”的技术闭环，不是证明系统可以自动认定或排除侵权。
...
- 用户界面：上传 AI 角色图、预览、返回数量、候选人物和参考图；
- 批量能力：目录导入脚本；
...
- 未检出显著候选不等于可以无风险发布；
- 当前系统不检查姓名、声音、角色文案、服装造型、具体摄影作品复制或授权合同；
- 正式上线前仍需扩充合法来源人物库，并以实际 AI 短剧角色图完成不同人群、画风、年龄、侧脸、妆容和生成模型域偏移的阈值标定；
- 技术输出必须进入人工审核流程，不应直接映射为“安全”或“侵权”。
